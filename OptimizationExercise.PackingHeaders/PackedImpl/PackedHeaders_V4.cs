...
{
    public sealed class PackedHeaders_V4 : IHeadersStructure<PackedHeaders_V4, PackedHeaders_V4.Enumerator>
    {
        public struct Enumerator : IEnumerator<HeaderNames>
        {
            private readonly PackedHeaders_V4 outer;

            private ulong nextBitMask;
            ...
            internal Enumerator(PackedHeaders_V4 outer)
            {
                Current = default;
                this.outer = outer;
            ...
            tryAgain:
                ...
                var bitfield = Unsafe.Add(ref outer.bitfield.bitfield0, currentBitfieldIndex);

                while (true)
                {
                    var bitIx = Helpers.NthSetBit_Intrinsics(bitfield, nextBitMask);
                    nextBitMask <<= 1;
                    ...
                    yieldedCount++;
                    Current = (HeaderNames)(bitIx + 64 * currentBitfieldIndex);
                    return true;
                }
        ...
        public void Set(HeaderNames name, string value)
        {
            ...
            BitfieldCalculation(nameAsByte, out var bitfieldIndex, out var bitIndex);
            ref ulong bitfield = ref GetBitfield(bitfieldIndex);

            // irritatingly, the JIT knows to turn this into a bt while the longer form... isn't
            var alreadySet = (bitfield & (1UL << bitIndex)) != 0;
            if (alreadySet)
            {
            ...
            var bitsSetBeforeBitfieldIndex = Helpers.BitsSetBeforeIndex_Intrinsics(bitfield, bitIndex);
        ...
        public bool TryGetValue(HeaderNames name, [NotNullWhen(returnValue: true)] out string? value)
        {
            ...
            // irritatingly, the JIT knows to turn this into a bt while the longer form... isn't
            var notSet = (bitfield & (1UL << bitIndex)) == 0;
            if (notSet)
            {
                value = null;
                return false;
            ...
            // figure out which _value_ index we're going to stash the value in
            var bitsSetBeforeBitfield = GetBitsSetBeforeBitfield(bitfieldIndex);
            var bitsSetBeforeBitfieldIndex = Helpers.BitsSetBeforeIndex_Intrinsics(bitfield, (byte)bitIndex);
        ...
        public static PackedHeaders_V4 CreateEmpty()
        => new PackedHeaders_V4();

        public static PackedHeaders_V4 CreateFromDictionary(Dictionary<HeaderNames, string> dict)
        {
        ...
        private bool TryShiftValuesDownFrom(int index)
        {
            ...
            var copyLen = (Constants.MaximumSetHeaders - 1) - index;
        ...
        => ref Unsafe.Add(ref bitfield.bitfield0, index);
        ...
        => Helpers.CountSetBits_Intrinsics(bitfield.bitfield0);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private byte Bitfield01SetBits()
        => (byte)(Helpers.CountSetBits_Intrinsics(bitfield.bitfield0) + Helpers.CountSetBits_Intrinsics(bitfield.bitfield1));

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void BitfieldCalculation(byte num, out byte bitfieldIndex, out byte bitIndex)
        {
            bitfieldIndex = (byte)Helpers.BitFieldExtract_Intrinsics(num, 6, 2);
            bitIndex = (byte)Helpers.ZeroHighBits_Intrinsics(num, 6);
        }
    }
}
