...
namespace OptimizationExercise.PackingHeaders.DictionaryImpl
{
    /// <summary>
    /// Implementation backed entirely by a Dictionary<HeaderNames, string?>
    /// </summary>
    public sealed class DictionaryHeaders : IHeadersStructure<DictionaryHeaders, Dictionary<HeaderNames, string>.KeyCollection.Enumerator>
    {
        private readonly Dictionary<HeaderNames, string> inner;
        ...
        private DictionaryHeaders(Dictionary<HeaderNames, string> inner)
        {
            this.inner = inner;
        }
        ...
        => inner.TryGetValue(header, out value);

        public Dictionary<HeaderNames, string>.KeyCollection.Enumerator GetEnumerator()
        => inner.Keys.GetEnumerator();

        public static DictionaryHeaders CreateEmpty()
        => new DictionaryHeaders(new Dictionary<HeaderNames, string>());

        public static DictionaryHeaders CreateFromDictionary(Dictionary<HeaderNames, string> headers)
        {
            ...
            return new DictionaryHeaders(new Dictionary<HeaderNames, string>(headers));
        }

        // basically just adding a null check for property purposes, inline this
        ...
        // this is the meat of the public set interface, inlining in each call is important
        [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
        private void SetImpl(HeaderNames header, string value)
        {
            if (inner.Count == Constants.MaximumSetHeaders)
            {
                throw new InvalidOperationException();
            }

            inner.Add(header, value);
        }

        // we want each property access to devolve to a dictionary lookup, so aggressively inline
        [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
        private string? GetOrDefault(HeaderNames header)
        => inner.TryGetValue(header, out string? value) ? value : null;
    }
}
