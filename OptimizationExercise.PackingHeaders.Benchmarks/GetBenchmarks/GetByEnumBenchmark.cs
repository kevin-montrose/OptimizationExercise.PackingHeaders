...
{
    /// <summary>
    /// Benchmarking reading headers by the <see cref="HeaderNames"/> enum.
    /// 
    /// Parameterized by the number of headers set, we read the full set of headers.
    /// </summary>
    [SimpleJob]
    public class GetByEnumBenchmark : ImplBenchmarkBase
    {
        ...
        private HeaderNames[] ToRead = Array.Empty<HeaderNames>();
        ...
        public override void GlobalSetup()
        {
            ...
            ToRead = Data.AllHeaders.ToArray();
            ...
            where V : struct, IEnumerator<HeaderNames>
        {
            for (var iter = 0; iter < Iterations; iter++)
            {
                ...
                foreach (var header in ToRead)
                {
                    if (headers.TryGetValue(header, out string? value))
                    {
                        StoreInto[nextIx] = value;
                        nextIx++;
                    }
                }
            }
